...
- **Project permission guard membership fallback**: `project_permission_guard!` now falls back to `ProjectAccessChecker::user_can_access_project()` when `effective_project_permissions()` returns `Ok(None)`, preserving legacy team-membership denial on project-scoped write endpoints while still allowing permission-specific narrowing.
