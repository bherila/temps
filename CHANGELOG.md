...
- **MariaDB explorer filter hardening**: Replaced raw `filters.where` SQL fragments with structured, parameterized MariaDB data explorer conditions so read-only explorer requests cannot inject subqueries against other databases or tables.
