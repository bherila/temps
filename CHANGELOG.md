...
- **Compose sandbox bypass closure**: hardened the compose security policy against a set of escapes found in review. The policy now rejects interpolated bind sources (`${HOST_ROOT:-/}`) that cannot be statically validated, service-level `extends` (which can import privileged settings from another file), `use_api_socket`, GPU device requests (`gpus`), and the remaining host namespace modes (`cgroup`, `userns_mode`, `uts`). Relative bind sources are now lexically normalized so `../../../../etc` and `/tmp/../etc` can no longer escape the project directory or the absolute-path block. Privileged build options (`build.privileged`, `build.entitlements`, `build.network: host`) are rejected before `docker compose build` runs them. Top-level named volumes whose `driver_opts.device` binds a forbidden host path are detected and any service mount referencing them is rejected, and host files exposed via `configs.*.file` / `secrets.*.file` are blocked. YAML merge keys (`<<`) and anchors are expanded before validation, and the security override is generated from the parsed `services` mapping (covering inline mappings and anchors) and is now applied **last** in the `docker compose up -f` order so user/preset overrides cannot weaken it. Security validation also runs as a preflight before the existing stack is torn down, so a rejected deploy no longer causes downtime.
