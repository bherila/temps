...
- **providers MariaDB query filters**: Reject subqueries and MariaDB error-producing XML functions in data-browser `where` filters and stop returning raw database error details, preventing `ExternalServicesRead` callers from using filter expressions to read data outside the selected table.
