...
/// - **Project scoping**: when `allowed_project_ids` is non-empty, the
///   operation must expose a recognized project selector (`project_id` or the
///   leading `/projects/{id}` path param). The selector value is validated ∈
///   `allowed_project_ids`. If the selector is absent and
///   `allowed_project_ids.len() == 1`, it is auto-filled with the single
///   accessible project.
...
pub fn build_request_parts(
    ...
    let has_project_scope_param = op
        .params
        .iter()
        .any(|param| is_project_scope_param(op, param));
    if !allowed_project_ids.is_empty() && !has_project_scope_param {
...
mod tests {
    ...
    fn non_project_id_path_param_is_rejected_for_project_scoped_call() {
        ...
        // resource id) cannot prove that the replay stays inside the chat's
        // project, so a project-scoped AI call must reject it instead of
        // forwarding a potentially instance-wide resource lookup.
