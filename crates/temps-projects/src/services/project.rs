...
// as microcores (1_000_000 = 1 CPU core); memory values are stored as MB. Only
// CPU *requests* (scheduling minimums) are defaulted -- CPU *limits* are
// intentionally left unset so new projects/environments run uncapped on CPU by
// default. Memory *limits* are defaulted for hosted websites (see
// DEFAULT_MEMORY_LIMIT below) because app containers are usually burstier and
// safer to cap by default on small single-node installs; these profiles are
// intentionally separate from the external database-service profiles.
