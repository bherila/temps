export const metadata = {
  ...
  description: 'Provision and manage PostgreSQL, Redis, and S3 storage directly within Temps—no third-party services required.',
  alternates: {
...
<Row>
  <Col>
    ...
    - **S3 Storage** — Object storage compatible with AWS S3 SDKs
...
Provision S3-compatible object storage for files, backups, and assets:
...
<Row>
  ...
  <Col>
    **Configuration Options:**
    ...
    | Option | Description | Default |
    |--------|-------------|---------|
    | `backend` | Storage backend: `rustfs`, `garage`, or `minio` | `rustfs` |
...
<Note>
RustFS is the default backend. Garage is modeled as an external provider
backend, and MinIO is available for compatibility with existing
...
| **S3 Storage** | ✅ S3-compatible | ⚠️ Vercel Blob (extra) | ❌ External only | ❌ External only |
