export const metadata = {
  ...
  description:
    'Provision PostgreSQL, Redis, MongoDB, and S3 storage as Docker containers managed by Temps. Link them to projects for automatic credential injection.',
  alternates: {
...
| **S3 Storage** | `rustfs/rustfs:1.0.0-alpha.78` | S3-compatible object storage (`rustfs` by default; `garage` and `minio` optional) |
...
| Parameter | Default | Editable after creation |
|-----------|---------|----------------------|
| `backend` | `rustfs` (`garage` or `minio` optional) | No (read-only) |
